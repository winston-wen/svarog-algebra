...
// $$\Delta_K = -pq$$.
static DELTA_K: LazyLock<Integer> = LazyLock::new(|| -(P.clone() * Q.clone()));

// $$\Delta_p = p^2 \Delta_K$$.
static DELTA_P: LazyLock<Integer> = LazyLock::new(|| P.clone().square() * DELTA_K.clone());
...
static G: LazyLock<QuadForm> = LazyLock::new(|| {
    ...
    let c = super::derive_c_from_abd(&a, &b, &DELTA_P);
    QuadForm(a, b, c)
});
...
static F: LazyLock<QuadForm> = LazyLock::new(|| {
    ...
    let b = P.clone();
    let c = derive_c_from_abd(&a, &b, &DELTA_P);
    QuadForm(a, b, c)
});

static ID: LazyLock<QuadForm> = LazyLock::new(|| {
...
pub static CTX: LazyLock<ClSettings> = LazyLock::new(|| ClSettings {
    p: &P,
    q: &Q,
    delta_k: &DELTA_K,
    delta_p: &DELTA_P,
