...
impl TrDiscriminant for Delta1827bit {
    ...
    fn Delta_k() -> &'static Integer {
        static DELTA_K: LazyLock<Integer> =
            LazyLock::new(|| -(Delta1827bit::p().clone() * Delta1827bit::q()));
        return &DELTA_K;
    }
    ...
    fn Delta_p() -> &'static Integer {
        static DELTA_P: LazyLock<Integer> =
            LazyLock::new(|| Delta1827bit::p().clone().square() * Delta1827bit::Delta_k());
        &DELTA_P
    }
    ...
    where
        Self: Sized + Clone,
    {
        static ID: LazyLock<QuadForm<Delta1827bit>> =
            LazyLock::new(|| QuadForm::new(1, 1).unwrap());
        return &ID;
    }
