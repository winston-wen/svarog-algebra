...
pub struct Delta1827 {}

impl TrDiscriminant for Delta1827 {
    // $$\Delta_k = -pq$$.
    fn Delta_k() -> &'static Integer {
        static DELTA_K: LazyLock<Integer> =
            LazyLock::new(|| -(Delta1827::p().clone() * Delta1827::q()));
        return &DELTA_K;
    }

    // $$\Delta_p = p^2 \Delta_K$$.
    fn Delta_p() -> &'static Integer {
        static DELTA_P: LazyLock<Integer> =
            LazyLock::new(|| Delta1827::p().clone().square() * Delta1827::Delta_k());
        &DELTA_P
    }
    ...
    where
        Self: Sized + Clone,
    {
        static ID: LazyLock<QuadForm<Delta1827>> = LazyLock::new(|| QuadForm::new(1, 1));
        return &ID;
    }
    ...
    where
        Self: Sized + Clone,
    {
        static F: LazyLock<QuadForm<Delta1827>> = LazyLock::new(|| {
            let a = Delta1827::p().clone().square();
            let b = Delta1827::p().clone();
            QuadForm::new(a, b)
        });
        return &F;
    }
    ...
    where
        Self: Sized + Clone,
    {
        static G: LazyLock<QuadForm<Delta1827>> = LazyLock::new(|| {
            // g.a
            let digits = String::new()
    ...
    fn L() -> &'static Integer {
        static L: LazyLock<Integer> = LazyLock::new(|| {
            let mut val = Delta1827::Delta_p().clone();
            val /= 4;
            val = val.sqrt();
            val = val.sqrt();
            val
