...
fn test_lhe() {
    ...
    '_test_add_ct: {
        ...
        let m_gt = Integer::from(&m1 + &m2).modulo(delta1792::p());
...
fn test_mta() {
    ...
    let lhs = (uji + vji) % delta1792::p();
    let rhs = (ki * wj) % delta1792::p();
    assert_eq!(lhs, rhs);
}
...
fn show_log2_of_order() {
    let ord_len = delta1792::order_g_approx().significant_bits();
