...
fn test_mta() {
    ...
    let lhs = (uji + vji) % cl_params::p();
    let rhs = (ki * wj) % cl_params::p();
    assert_eq!(lhs, rhs);
}
...
fn bench_exp() {
    ...
    for _ in 0..n {
        ...
        let _ = cl_params::generator_Delta_K().exp(&e);
...
fn show_log2_of_order() {
    let ord_len = cl_params::order_g_approx().significant_bits();
